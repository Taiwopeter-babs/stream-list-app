...
function Movie(prop: IMovieProp) {
            ...
            <Suspense fallback={<div>LOADING...</div>}>
                <div className={styles.imgContainer}>
                    <Image
                        ...
                        height={300}
                    />
                    {/* <p className={styles.overview} style={{ "fontSize": 12 }}>{`(${prop.overview.slice(0, 50)}...)`}</p> */}
                    {/* <p className={styles.overview}>{prop.overview}</p> */}
                </div>
            </Suspense>
            <div className={styles.movieTitle}>{title}</div>

            <div className={styles.overview} style={{ "fontSize": 12 }}>
                Overview:<b>&nbsp;{`${prop.overview.slice(0, 40)}...`}</b>
            </div>
